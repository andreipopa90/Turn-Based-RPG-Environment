        ...
        onHitField: function (target, source) {
            ...
            if (this.gameType === "freeforall") {
                ...
                for (var _i = 0, sideConditions_1 = sideConditions; _i < sideConditions_1.length; _i++) {
                    ...
                    if (!someCondition)
                        continue;
                    _a = __spreadArray([], rotatedSides, true), sides[0].sideConditions[id] = _a[0], sides[1].sideConditions[id] = _a[1], sides[2].sideConditions[id] = _a[2], sides[3].sideConditions[id] = _a[3];
...
var result = [];
for (const [key, value] of Object.entries(exports.Moves)) {
  var obj = new Object();
  obj.name=value.name;
  obj.accuracy = value.accuracy;
  obj.basePower = value.basePower;
  obj.category = value.category;
  obj.pp = value.pp;
  obj.target = value.target;
  obj.type = value.type;
  if (value.category == "Status" && "boosts" in value) {
    obj.boosts = value.boosts;
    result.push(obj);
  }
...
fs.writeFile('moves.json', jsonResult, (err) =>{
  if (err) {
    throw err;
  }
